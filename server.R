...
shinyServer(function(input, output, session) {
  ...
  data <- reactive({
    if (is.null(input$file)) {
      ...
      fileText <-
        paste(readLines(filePath, encoding = "UTF-8"), collapse = "\n")
  ...
  output$phrasextractnoun <- renderDataTable({
  ...
  ######################### Verb Extraction###########################
  output$phrasextractverb <- renderDataTable({
  ...
  ########################Collocations################################
  output$collocations <- renderDataTable({
    collocateddata <- keywords_collocation(
      x = anndata(),
      term = "token",
      group = c("doc_id",
  ...
  output$cooccurrences_noun_adj <- renderDataTable({
    validate(need(
      length(input$xposchoice) > 2 ,
      "Please select atleast 2 xpos tags"
    ))
    ...
    cooccurrences_noun_adj <- cooccurrence(
      if (input$lang == "en") {
        ...
        x = subset(anndata(), upos %in% postags())
      },
      term = "lemma",
  ...
  output$gen_cooccur <- renderDataTable({
    adata = anndata()
    validate(need(
      length(input$xposchoice) > 2 ,
      "Please select atleast 2 xpos tags"
  ...
  output$cooccur_skips_ngrams <- renderDataTable({
    validate(need(
      length(input$xposchoice) > 2 ,
      "Please select atleast 2 xpos tags"
    ))
    adata = anndata()
    if (input$lang == "en") {
      cooccur_skips_ngrams <- cooccurrence(
        x = adata$lemma,
  ...
  output$plot1 = renderPlot({
    validate(need(
      length(input$xposchoice) > 2 ,
      "Please select atleast 3 xpos tags"
    ))
    cooccurrences_noun_adj <- cooccurrence(
      if (input$lang == "en") {
        x = subset(anndata(), xpos %in% postags())
